...
module SonicPi
  class Scale < WrappingArray
    ...
    def self.resolve_degree_index(degree)
      # Defensive: handle nil input
      if degree.nil?
        raise InvalidDegreeError, "Scale degree cannot be nil"
      end
      ...
      if num.nil?
        raise InvalidDegreeError, "Invalid scale degree #{degree.inspect}, could not parse as number or roman numeral"
      end

      if num <= 0
        raise InvalidDegreeError, "Invalid scale degree #{degree.inspect}, scale degree must be greater than 0 (got #{num})"
      end
    ...
    def self.resolve_degree(degree, tonic, scale)
      ...
      raise ArgumentError, "Scale cannot be nil" if scale.nil?

      scale_obj = Scale.resolve_scale(tonic, scale, 1)
      augmentation = 0

      # Parse augmentation prefixes for non-numeric degrees
      unless degree.is_a?(Numeric)
        degree_str = degree.to_s.downcase.strip

        # Defensive: check for empty string
        if degree_str.empty?
          raise InvalidDegreeError, "Degree string cannot be empty"
        end

        if degree_str.start_with?('p')
          augmentation = 0
          degree_str = degree_str[1..-1]
        elsif degree_str.start_with?('aa')
          augmentation = 2
          degree_str = degree_str[2..-1]
        elsif degree_str.start_with?('a')
          augmentation = 1
          degree_str = degree_str[1..-1]
        elsif degree_str.start_with?('dd')
          augmentation = -2
          degree_str = degree_str[2..-1]
        elsif degree_str.start_with?('d')
          augmentation = -1
          degree_str = degree_str[1..-1]
        end
      ...
      # Defensive: ensure scale has notes
      if scale_obj.notes.nil? || scale_obj.notes.empty?
        raise InvalidScaleError, "Scale has no notes"
      end

      degree_index = resolve_degree_index(degree)
      scale_length = scale_obj.notes.length - 1

      # Defensive: prevent division by zero
      if scale_length <= 0
        raise InvalidScaleError, "Scale must have at least 2 notes"
      end

      octave, index = degree_index.divmod(scale_length)

      # Defensive: validate index
      if index < 0 || index >= scale_obj.notes.length
        raise InvalidDegreeError, "Computed index #{index} out of range for scale"
      end

      scale_obj.notes[index] + (octave * 12) + augmentation
    end
    ...
    def self.resolve_scale(tonic, name, num_octaves)
      ...
      raise ArgumentError, "Number of octaves cannot be nil" if num_octaves.nil?

      # Defensive: validate num_octaves
      ...
      tonic_key = tonic.is_a?(String) ? tonic.to_sym : tonic
      name_key = name.is_a?(String) ? name.to_sym : name

      # Check cache
      cached = @@scale_cache[tonic_key][name_key][num_octaves]
      return cached unless cached.nil?

      # Create new scale
      scale = Scale.new(tonic_key, name_key, num_octaves)
      @@scale_cache[tonic_key][name_key][num_octaves] = scale
      scale
    end
    ...
    def initialize(tonic, name, num_octaves=1)
      ...
      intervals = SCALE[name_sym]
      raise InvalidScaleError, "Unknown scale name: #{name.inspect}" unless intervals

      # Defensive: validate intervals array
      unless intervals.is_a?(Array)
        raise InvalidScaleError, "Scale intervals must be an array for #{name.inspect}"
      end

      if intervals.empty?
        raise InvalidScaleError, "Scale intervals cannot be empty for #{name.inspect}"
      end

      # Multiply intervals for multiple octaves
      intervals = intervals * num_octaves_int

      # Defensive: resolve tonic
      begin
        current = Note.resolve_midi_note(tonic)
      rescue Note::InvalidNoteError => e
        # Re-raise InvalidNoteError to preserve error type
        raise e
      rescue => e
        raise InvalidScaleError, "Invalid tonic note #{tonic.inspect}: #{e.message}"
      end

      raise InvalidScaleError, "Could not resolve tonic #{tonic.inspect}" if current.nil?
      ...
      intervals.each_with_index do |interval, idx|
        ...
        unless interval.is_a?(Numeric)
          raise InvalidScaleError, "Interval at index #{idx} must be numeric, got #{interval.class}"
        end

        if interval < 0
          raise InvalidScaleError, "Interval at index #{idx} must be non-negative, got #{interval}"
        end
